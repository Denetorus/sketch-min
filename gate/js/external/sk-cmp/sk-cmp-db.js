...
export class SCInputBoxDB extends SCInputBox{
    ...
    getInputItemByType(){
        ...
        if (this.type==='object'){
            ...
            const props = {}
            const fieldDescription = this.object.fieldDescriptions[this.fieldKey];

            if (fieldDescription.hasOwnProperty(""))
                if (fieldDescription.hasOwnProperty('refFilters'))
                    props.filters = this.object.fieldDescriptions[this.fieldKey];

            if (fieldDescription.hasOwnProperty('refSorts'))
...
export class SCUListDB extends SCUList{
    ...
    renderRow(row){
        this.items.push(
            this.renderCell(row['ref'], row['presentation'])
        )
    }
